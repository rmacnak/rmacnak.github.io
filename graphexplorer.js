...
function showTables(nodes) {
  ...
  let example1 = document.createElement("a");
  example1.href = "?snapshot=HelloApp.vfuel.json";
  example1.textContent = "HelloApp";
  let example2 = document.createElement("a");
  example2.href = "?snapshot=CompilerApp.vfuel.json";
  example2.textContent = "CompilerApp";
  let example3 = document.createElement("a");
  example3.href = "?snapshot=TestRunner.vfuel.json";
  example3.textContent = "TestRunner";
  let example4 = document.createElement("a");
  example4.href = "?snapshot=BenchmarkRunner.vfuel.json";
  example4.textContent = "BenchmarkRunner";
  let example5 = document.createElement("a");
  example5.href = "?snapshot=HopscotchIDE.vfuel.json";
  example5.textContent = "HopscotchIDE";
  ...
  if (graph) {
    ...
    column.appendChild(example1);
    column.appendChild(example2);
    column.appendChild(example3);
    column.appendChild(example4);
    column.appendChild(example5);
  }
...
(function(){
  ...
  request.responseType = "utf-8";
  request.onload = function (event) {
    if (request.status != 200) {
      document.write("Failed to load " + path + ": " +
    ...
    g.compute(false);
    // showTables([graph.getRoot()]);
    showDominatorTree(graph.toggleMerge(), graph.getRoot());
  };
  request.send();
})()
